...
export const handlers = [
  ...
  rest.post(makeUrl('/api/issues/:number/comments'), async (req, res, ctx) => {
    try {
      await handleErrorDelay(req)
    } catch {
      return res(ctx.status(500), ctx.json({ error: 'Error in request' }))
  ...
  rest.put(makeUrl('/api/issues/:number'), async (req, res, ctx) => {
    try {
      await handleErrorDelay(req)
    } catch {
      return res(ctx.status(500), ctx.json({ error: 'Error in request' }))
    ...
    if (body.labels) {
      issue.labels = body.labels
        .map((l) => labels.find((lbl) => lbl.id === l)?.id)
        .filter(Boolean)
    }
    if (body.dueDate) {
  ...
  rest.post(makeUrl('/api/issues/:number/complete'), async (req, res, ctx) => {
    try {
      await handleErrorDelay(req)
    } catch {
      return res(ctx.status(500), ctx.json({ error: 'Error in request' }))
  ...
  rest.post(makeUrl('/api/issues'), async (req, res, ctx) => {
    try {
      await handleErrorDelay(req)
    } catch {
      return res(ctx.status(500), ctx.json({ error: 'Error in request' }))
  ...
  rest.post(makeUrl('/api/labels'), async (req, res, ctx) => {
    ...
    if (labels.find((l) => l.name === body.name))
      return res(ctx.status(400), ctx.json({ message: 'Label already exists' }))
